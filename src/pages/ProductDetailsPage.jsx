...
import { ChevronLeft, Star, Heart, ShoppingCart, Minus, Plus, Truck, Shield, RotateCcw } from 'lucide-react';
...
const ProductDetailsPage = () => {
  ...
  // Generate additional product details based on product type
  ...
  const StarRating = ({ rating, size = 'medium' }) => {
    ...
    for (let i = 0; i < 5; i++) {
      if (i < fullStars) {
        stars.push(
          <Star key={i} className={`${size === 'small' ? 'w-4 h-4' : 'w-5 h-5'} fill-amber-400 text-amber-400`} />
        );
      } else if (i === fullStars && hasHalfStar) {
        stars.push(
          <Star key={i} className={`${size === 'small' ? 'w-4 h-4' : 'w-5 h-5'} fill-amber-200 text-amber-400`} />
        );
      } else {
        stars.push(
          <Star key={i} className={`${size === 'small' ? 'w-4 h-4' : 'w-5 h-5'} text-gray-300`} />
        );
      }
    }
    ...
    return (
      <div className="min-h-screen bg-[#E6E5E1] flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-light text-gray-800 mb-2">Product Not Found</h2>
          <p className="text-gray-600 mb-6">The coffee you're looking for doesn't exist.</p>
          <button 
            onClick={handleBack}
                ...
                {products
                  ...
                  .map((relatedProduct) => (
                    ...
                    >
                      ...
                      />
                      <div className="w-full h-20 bg-gradient-to-br from-amber-100 to-amber-200 rounded flex items-center justify-center text-amber-600 text-2xl mb-2 hidden">
                        ☕
                      </div>
                      <div className="text-xs font-medium text-gray-800 mb-1 line-clamp-2">
                        {relatedProduct.title}
                      </div>
                      <div className="text-sm text-amber-600">₹ {relatedProduct.price}</div>
                    </div>
                  ))}
              </div>
            </div>
